...
int cekpapan()//mengecek apakah kotak masih ada yang kosong
{
	int row, col;
...
void two_player()
{
	do
	{
		...
		player_move(2);
	}while (!cekpapan());//ulangi selama papan masih ada yang kosong
...
void player_move(int player)
{
  ...
  do
  {
  	if(cekpapan()){//digunakan untuk mengecek kondisi ketika mendapatkan sos di kotak terakhir
  		return;
	  }
	gambarpapan(); 
...
void howtoplay(){
	...
	if(i==1){
		...
		inispapan();
		two_player(); 
		if(cekpapan()){//jika papan sudah penuh
			gambarpapan();
			winner();
		}      
	}else if(i==0){
